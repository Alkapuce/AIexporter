...
export async function materializeAiStudioDomImageMarkdown(
  ...
  while ((match = markdownImagePattern.exec(markdown)) !== null) {
    ...
    if (!/^https?:\/\//i.test(target) && !/^blob:/i.test(target)) {
      output += fullMatch;
      continue;
    }
    ...
    try {
      ...
      const mimeType = inferAiStudioImageMimeType(target, response);
      const buffer = await response.arrayBuffer();
      output += `![${altText}](data:${mimeType};base64,${encodeBytesToBase64(new Uint8Array(buffer))})`;
      continue;
    } catch {
      output += fullMatch;
    }
  ...
  return {
    ...
    messages: await Promise.all(
      bundle.messages.map(async (message) => ({
        ...message,
        markdown: await materializeAiStudioDomImageMarkdown(message.markdown),
      })),
    ),
  };
}
...
async function extractCurrentAiStudioConversation(
  ...
  if (isLikelyTitleOnlyBundle(bundle)) {
    ...
    await log("info", "Extracted AI Studio conversation via DOM fallback.", {
      ...
      sourceId,
      messageCount: materializedBundle.messages.length,
    });
  }
