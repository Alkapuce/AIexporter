...
import { canRunAutomaticArtifactSync, isAutomaticArtifactSyncDue, type ArtifactSyncState } from "./artifact-sync-state";
  ...
  try {
    ...
    if (canRunAutomaticArtifactSync(queueState) && isAutomaticArtifactSyncDue(syncState.lastCompletedAt)) {
      await deps.writeBackgroundLog(
        "background.artifact",
        "info",
        "Running weekly automatic artifact sync before scheduler ticks.",
        {
