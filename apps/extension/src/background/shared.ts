...
export const STALE_BUSY_WORKER_RECOVERY_MS = 90_000;
export const GOOGLE_CHALLENGE_COOLDOWN_MS = 30 * 60_000;
...
export function buildConversationFolderName(title: string | undefined, sourceId: string): string {
  const rawTitle = normalizeConversationTitle(title, sourceId) ?? sourceId;
...
export function buildArchivePrefix(bundle: ConversationBundle, revision: string): string {
  ...
  const conversationFolder = buildConversationFolderName(resolvedTitle.title, bundle.sourceId);
  return `AIexporter/${sanitizePathSegment(bundle.platform)}/${conversationFolder}`;
}

export function buildArchivedRevisionPrefix(bundle: ConversationBundle, revision: string): string {
  const resolvedTitle = resolveBundleTitle(bundle);
  const conversationFolder = buildConversationFolderName(resolvedTitle.title, bundle.sourceId);
  const revisionFolder = buildRevisionFolderName(revision);
  return `AIexporter/Archive/${sanitizePathSegment(bundle.platform)}/${conversationFolder}/${revisionFolder}`;
}

export function buildArtifactBaseName(bundle: ConversationBundle): string {
  const { title } = resolveBundleTitle(bundle);
  const raw = title.trim() || bundle.sourceId;
