...
export function createBackgroundServiceRuntime(): BackgroundServiceRuntime {
  ...
  function getDiscoveryCadenceAnchorMs(service: PlatformServiceState): number | undefined {
    ...
    if (service.stats.discoveredTotal > 0) {
      return parseTimestampMs(service.lastExportAt);
    }
  ...
  async function applyDiscoveryBatch(
    ...
    options: {
      priority: ExportQueueItem["priority"];
    ...
    if (queueableEvents.length > 0) {
      await updateQueueStateWithDerived((current) => ({
        ...
        items: queueableEvents.reduce(
          (items, event) =>
            mergeDiscoveryEvent(items, event, {
              kind: "export",
  ...
  async function queuePassiveDiscoveryEvents(events: DiscoveryEvent[]): Promise<{ total: number; queued: number }> {
    ...
    const deduped = Array.from(
      events
        .reduce(
          (map, event) => map.set(`${event.platform}:${event.sourceId}`, event),
          new Map<string, DiscoveryEvent>(),
        )
        .values(),
    );
  ...
  async function runPlatformDiscoverySweep(
    ...
    if (inFlightSweep) {
      await writeBackgroundLog(
        ...
        "debug",
        "Joined existing platform discovery sweep instead of starting a duplicate run.",
        {
          platform,
          mode,
        },
    ...
    const runPromise = (async () => {
      ...
      if (getPlatformDiscoveryRunCount(platform) > 0) {
        await writeBackgroundLog(
          "background.discovery",
          "debug",
          "Skipped platform discovery sweep because one is already running.",
          {
            platform,
            mode,
          },
      ...
      try {
        ...
        if (!selectedPayloads && bestPayloads.length > 0) {
          ...
          await traceLog(
            "warn",
            "Falling back to the best partial Gemini discovery batch collected before retry exhaustion.",
            {
        ...
        if (!selectedPayloads) {
          throw lastAttemptError instanceof Error
            ? lastAttemptError
            : new Error(`${platform} discovery sweep produced no payloads.`);
        }
  ...
  async function resolveWorkerTab(
    ...
    if (config.reuseWorkerTabs && worker.tabId) {
      try {
        ...
        await browser.tabs.update(worker.tabId, {
          url: navigableUrl,
          active: false,
          pinned: true,
        });
    ...
    try {
      ...
      if (queueStillProcessing || workerStillBusy) {
        ...
        await writeBackgroundLog(
          "background.queue",
          "warn",
          "Reconciled worker state after completion because a busy or processing lease remained.",
          {
      ...
      if (shouldAutoRetryReceiver) {
        await writeBackgroundLog(
          "background.queue",
          "warn",
          "Worker receiver was unavailable, requeued queue item for retry.",
          {
            code: errorCode,
            platform,
            workerId: worker.workerId,
        ...
        await writeBackgroundLog(
          "background.queue",
          "warn",
          "Google platform challenge detected, cooling down before retrying the queue item.",
          {
            code: errorCode,
            platform,
            workerId: worker.workerId,
  ...
  async function maybeRunPlatformDiscovery(platform: SourcePlatform, queueState: QueueState): Promise<void> {
    ...
    if (workerSnapshot.ghostBusyWorkers.length > 0) {
      await writeBackgroundLog(
        "background.discovery",
        "warn",
        "Ignoring ghost busy workers while deciding whether discovery can run.",
        {
    ...
    if (isGooglePlatform(platform) && nextPlannedAt > now && isChallengeErrorCode(undefined, service.lastError ?? "")) {
      ...
      await writeBackgroundLog(
        "background.discovery",
        "debug",
        "Google platform discovery is cooling down after a challenge page was detected.",
        {
    ...
    if (platformStartupCatchup.has(platform)) {
      ...
      if (shouldPreferQueueDrainBeforeCatchup) {
        await writeBackgroundLog(
          "background.discovery",
          "debug",
          "Deferred Gemini catch-up discovery until the first discovered export backlog drains.",
          {
      ...
      if (shouldForceHistoryCatchup) {
        await writeBackgroundLog(
          "background.discovery",
          "info",
          "Running Gemini catch-up discovery before draining the full export backlog because the previous bootstrap was partial.",
          {
      ...
      if (dueAt > now) {
        ...
        await writeBackgroundLog(
          "background.discovery",
          "debug",
          "Skipped startup discovery because the platform discovery cadence is still fresh.",
          {
            code: "discovery.startup_fresh_skip",
            platform,
            lastDiscoveryAt: service.lastDiscoveryAt,
            lastExportAt: service.lastExportAt,
            nextPlannedAt: new Date(dueAt).toISOString(),
            discoveredTotal: service.stats.discoveredTotal,
          },
        );
        return;
    ...
    if (shouldForceHistoryCatchup) {
      await writeBackgroundLog(
        "background.discovery",
        "info",
        "Running Gemini catch-up discovery because the current history set is still far below the previous high-water mark.",
        {
          code: "discovery.gemini_partial_catchup",
          platform,
          discoveredTotal: service.stats.discoveredTotal,
  ...
  async function maybeStartNextPlatformWorker(platform: SourcePlatform, queueState: QueueState): Promise<void> {
    ...
    if (isGooglePlatform(platform) && service.activeDiscoveryTabs > 0) {
      await writeBackgroundLog(
        "background.queue",
        "debug",
        "Worker start delayed until Google discovery sweep finishes.",
        {
          platform,
    ...
    if (workerSnapshot.ghostBusyWorkers.length > 0) {
      await writeBackgroundLog(
        "background.queue",
        "warn",
        "Ignoring ghost busy workers while deciding whether another worker can start.",
        {
    ...
    if (!nextItem) {
      await writeBackgroundLog(
        "background.queue",
        "debug",
        "Worker start skipped because no pending queue item was found.",
        {
          platform,
          queue: summarizeQueueItems(queueState.items),
        },
  ...
  async function cleanupIdleWorkerLeases(platform: SourcePlatform, options: { force?: boolean } = {}): Promise<number> {
    ...
    await writeBackgroundLog(
      "background.queue",
      "info",
      "Closed idle worker tabs after the platform returned to idle.",
      {
  ...
  async function runPlatformTick(platform: SourcePlatform): Promise<void> {
    ...
    if (rehydrated > 0) {
      await writeBackgroundLog(
        "background.queue",
        "info",
        "Filled missing queue items from pending conversation index entries before running the platform tick.",
        {
  ...
  async function clearPlatformLocalRecords(platform: SourcePlatform): Promise<QueueState> {
    ...
    try {
      ...
      await writeBackgroundLog(
        "background.cleanup",
        "warn",
        "Best-effort artifact sync failed after clearing local records.",
        {
          platform,
