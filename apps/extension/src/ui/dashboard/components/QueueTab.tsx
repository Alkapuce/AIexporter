  ...
  return (
    <div style={cardStyle}>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "1.3fr 0.7fr 0.8fr 0.8fr 0.8fr 0.8fr",
          gap: 10,
          marginBottom: 12,
        }}
      >
        ...
        <select
          style={inputStyle}
          value={sortKey}
          onChange={(event) => onSortKeyChange(event.target.value as DashboardQueueSortKey)}
        >
        ...
        <select
          style={inputStyle}
          value={sortDirection}
          onChange={(event) => onSortDirectionChange(event.target.value as DashboardSortDirection)}
        >
            ...
            return (
              ...
              >
                <div style={{ display: "grid", gap: 4 }}>
                  ...
                  <div style={{ color: "var(--aiexporter-text-muted-color)", fontSize: 11 }}>
                    {t("queue.websiteTime")}: {formatWebsiteTime(conversationEntry)}
                  </div>
                  <div style={{ color: "var(--aiexporter-text-muted-color)", fontSize: 11 }}>
                    {t("queue.lastSeenAt")}: {formatTimestamp(conversationEntry?.lastSeenAt)}
                  </div>
                  <div style={{ color: "var(--aiexporter-text-muted-color)", fontSize: 11 }}>
                    {t("queue.lastExportAt")}: {formatTimestamp(latestArtifact?.exportedAt)}
                  </div>
                  <div style={{ color: "var(--aiexporter-text-muted-color)", fontSize: 11 }}>
                    {t("queue.discoveredAt")}: {formatTimestamp(item.discoveredAt)}
                  </div>
                  {item.resultRevision ? (
                    <div style={{ color: "var(--aiexporter-text-muted-color)", fontSize: 11 }}>
                      {t("queue.resultRevision")}: {item.resultRevision.slice(0, 12)}
                    </div>
                  ) : null}
                  {item.skipReason === "latest_exists" ? (
                    <div style={{ color: "var(--aiexporter-button-accent-background)", fontSize: 11 }}>
                      {t("queue.skipReason.latest_exists")}
                    </div>
                  ) : null}
                  {item.errorCode ? (
                    <div style={{ color: "var(--aiexporter-danger-text-color)", fontSize: 11 }}>
                      {t("queue.errorCode")}: {item.errorCode}
                    </div>
                  ) : null}
                  {item.lastError ? (
                    <div style={{ color: "var(--aiexporter-danger-text-color)", fontSize: 11 }}>{item.lastError}</div>
                  ) : null}
                </div>
                <div style={{ display: "grid", gap: 4, fontSize: 12 }}>
                  <div>
                    {t("queue.priority")}: {item.priority}
                  </div>
                  <div>
                    {t("queue.attempts")}: {item.attempts}
                  </div>
                </div>
                <div style={{ display: "grid", gap: 4, fontSize: 12 }}>
                  <div>
                    {t("overview.status")}: {getQueueStatusLabel(item.status, t)}
                  </div>
                  <div>
                    {t("queue.updatedAt")}: {formatTimestamp(item.updatedAt)}
                  </div>
                </div>
                <div style={{ fontSize: 12 }}>
                  {conversationEntry?.latestSourceUpdatedLabel ?? t("common.notAvailable")}
                </div>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
                  <ActionButton
                    style={{ background: "var(--aiexporter-button-primary-background)" }}
                    disabled={busy}
                    onClick={() => onOpenSource(item.event.url)}
                  >
                    {t("common.openSource")}
                  </ActionButton>
                  {canOpenArtifacts ? (
